...
class Prey:
	...
	def __init__(self, x_max, y_max):
		...
		self.current_X = random.randint(0, Prey.VECTORS_MODULO-1)
		self.velocity = self.X_vectors[self.current_X]
	...
	def next_iteration(self):
		...
		if (self.position[0] < Prey.BOUND_SIZE):
			self.current_X = random.randint(-2, 2)
			self.velocity = self.X_vectors[self.current_X]
		elif (self.x_max - self.position[0] < Prey.BOUND_SIZE):
			self.current_X = random.randint(4, 8)
			self.velocity = self.X_vectors[self.current_X]
		elif (self.position[1] < Prey.BOUND_SIZE):
			self.current_X = random.randint(1, 5)
			self.velocity = self.X_vectors[self.current_X]
		elif (self.y_max - self.position[1] < Prey.BOUND_SIZE):
			...
			vector_offset = np.random.choice(list(range(0, 7)), 1, p=Prey.velocity_probabilities)[0]
			...
			self.current_X = (self.current_X + vector_offset) % Prey.VECTORS_MODULO
			self.velocity = self.X_vectors[self.current_X]
